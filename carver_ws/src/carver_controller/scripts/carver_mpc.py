...
class CombinedController(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f"Combined Controller initialized. Gamma_norm: {math.degrees(self.gamma_norm):.2f}Â°")
    ...
    def calculate_cross_track_error(self, segment_idx):
        ...
        # Vector Projection: proj = (ap Â· ab) / (ab Â· ab)
    ...
    def calculate_path_angle_gamma(self, target_segment_idx):
        """
        Calculates the current angle gamma (Î³) between path sections 
    ...
    def calculate_combined_steer(self):
        ...
        # Normalize gamma (Î³ / Î³_norm) [cite: 123]
        ...
        self.get_logger().info(
            f"k_pp: {k_pp:.2f} | k_st: {k_st:.2f} | Gamma: {math.degrees(gamma):.2f}Â° "
            f"| d_pp: {math.degrees(delta_pp):.2f}Â° | d_st: {math.degrees(delta_st):.2f}Â° "
            f"| Steer: {self.steering_sign * math.degrees(steer):.2f}Â°",
            throttle_duration_sec=0.5,
        )
        return steer

    def control_loop(self):
        if not self.controller_enabled or len(self.waypoints) == 0:
            return
        
        # Check if reached final waypoint (optional: for loop termination)
